...
declare
  ...
  treatment_start date := current_date;
begin
  if not exists (
  ...
  select new_medication_id,
    treatment_start::timestamptz + (floor(i / frequency) * interval '1 day') + ((8 + ((16.0 / frequency) * (i % frequency))) * interval '1 hour')
  ...
  if follow_up_input is not null then
    ...
    values (new_episode_id, target_member_id, follow_up_input ->> 'title', now() + interval '14 days', target_record_id);
  end if;
