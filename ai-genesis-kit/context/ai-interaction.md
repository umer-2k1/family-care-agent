...
> **This ai-genesis-kit is an overlay layer**, added on top of an already-scaffolded
...
@ai-genesis-kit/context/current-feature.md.
...
1. **Spec** - Optionally run `/brief` first for a read-only preview of the next
   ...
   @ai-genesis-kit/context/current-feature.md, then review it together before any code.
...
10. **Safety + log** - `/complete` first checks the active spec, branch, changed
   ...
   workflow files changed. Then it archives the spec to `ai-genesis-kit/history/features/NN-name.md` (or
   `ai-genesis-kit/history/fixes/`), checks the feature off in `ai-genesis-kit/build-plan.md`, and
   resets `ai-genesis-kit/context/current-feature.md` to its stub.
...
- For visual or replication features (recreating a design, matching a mockup),
  work from a reference image stored in `ai-genesis-kit/reference/`, not a prose
