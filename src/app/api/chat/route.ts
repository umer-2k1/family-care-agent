...
export async function POST(request: Request) {
  ...
  console.info(`[chat] Request started threadId=${threadId}`);
  try {
    ...
    console.info(`[chat] Request completed threadId=${threadId} intent=${result.intent}`);
    return Response.json({ intent: result.intent, response: result.response, toolResult: result.toolResult, threadId, mode: "configured" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "The care assistant is unavailable.";
    console.error(`[chat] Request failed threadId=${threadId}: ${message}`);
